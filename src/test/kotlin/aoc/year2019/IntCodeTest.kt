...
internal class IntCodeTest {
    ...
    fun test_examples_day5_part2_comparisons() {
        ...
        assertEquals(listOf(1L), computer.runUntilHalt(listOf(8)))
        computer.reset()
        assertEquals(listOf(0L), computer.runUntilHalt(listOf(18)))

        // input < 8
        computer = IntCode(arrayOf(3, 9, 7, 9, 10, 9, 4, 9, 99, -1, 8))
        assertEquals(listOf(1L), computer.runUntilHalt(listOf(7)))
        computer.reset()
        assertEquals(listOf(0L), computer.runUntilHalt(listOf(8)))
    ...
    fun test_examples_day5_part2_jumps() {
        ...
        assertEquals(listOf(0L), computer.runUntilHalt(listOf(0)))
        computer.reset()
        assertEquals(listOf(1L), computer.runUntilHalt(listOf(110)))
    ...
    fun test_examples_day5_part2_larger_example() {
        ...
        assertEquals(listOf(999L), computer.runUntilHalt(listOf(-15)))
        computer.reset()
        assertEquals(listOf(1000L), computer.runUntilHalt(listOf(8)))
        computer.reset()
        assertEquals(listOf(1001L), computer.runUntilHalt(listOf(12345)))
        computer.reset()
    ...
    @Test
    fun test_examples_day9_part1() {
        val quineProgram =
            listOf(109, 1, 204, -1, 1001, 100, 1, 100, 1008, 100, 16, 101, 1006, 101, 0, 99).map { it.toLong() }
        val quineComputer = IntCode(quineProgram.toTypedArray())
        val res = quineComputer.runUntilHalt()
        assertEquals(quineProgram, res)

        val longProgram = listOf(1102, 34915192, 34915192, 7, 4, 7, 99, 0).map { it.toLong() }
        val longComputer = IntCode(longProgram.toTypedArray())
        val res2 = longComputer.runUntilHalt()
        ...
        assertEquals(1125899906842624L, res3.first())
    }
}